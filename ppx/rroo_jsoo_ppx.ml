  ...
  ; pvb_expr=
      recursivelyMakeNamedArgsForExternal namedArgListWithKeyAndRef
        (Typ.arrow nolabel
           ...
           ; ptyp_attributes= [] }
           propsType)
  ; pvb_attributes= [({txt= "bs.obj"; loc}, PStr [])] }
...
let makePropsType ~loc namedTypeList =
  Typ.mk ~loc
    (Ptyp_constr
       ...
       , [ { ptyp_desc=
               Ptyp_object
                 (List.map (makeObjectField loc) namedTypeList, Closed)
           ; ptyp_loc= loc
           ; ptyp_attributes= [] } ] ))
  ...
  let transformLowercaseCall mapper loc attrs callArguments id =
    ...
    let createElementCall =
      ...
      | {pexp_desc= Pexp_array _} ->
          raise
            (Invalid_argument
               "A spread + an array literal as a DOM element's children would \
                cancel each other out, and thus don't make sense written \
                together. You can simply remove the spread and the array \
                literal.")
      ...
      | {pexp_attributes}
        when pexp_attributes
          ...
          raise
            (Invalid_argument
               "A spread + a JSX literal as a DOM element's children don't \
                make sense written together. You can simply remove the spread.")
      | _ ->
          ...
          if hasAttrOnBinding binding then
            ...
            let modifiedBinding binding =
              ...
              let wrapExpressionWithBinding expressionFn expression =
                { (filterAttrOnBinding binding) with
              ...
              let rec spelunkForFunExpression expression =
                ...
                | {pexp_desc= Pexp_fun _} ->
                    ( (fun expressionDesc ->
                        {expression with pexp_desc= expressionDesc} )
                    , expression )
                ...
                | {pexp_desc= Pexp_let (recursive, vbs, returnExpression)} ->
                    ...
                    ( (fun expressionDesc ->
                        { expression with
                          pexp_desc=
                            Pexp_let
                              (recursive, vbs, wrapExpression expressionDesc)
                        } )
                ...
                | { pexp_desc=
                    ...
                    ( (fun expressionDesc ->
                        { expression with
                          pexp_desc=
                            Pexp_sequence
                              (wrapperExpression, wrapExpression expressionDesc)
                        } )
                    , realReturnExpression )
                | _ ->
                    raise
            ...
            let makeLet innerExpression
              ...
              let labelString =
                ...
                | label when isOptional label || isLabelled label ->
                    getLabel label
                | _ ->
                    raise (Invalid_argument "This should never happen")
              in
              let expression =
                Exp.apply ~loc
              ...
              let expression =
                match default with
                | Some default ->
                    Exp.match_ expression
                      [ Exp.case
                          (Pat.construct {loc; txt= Lident "Some"}
                ...
                | None ->
                    expression
              in
              let letExpression = Vb.mk pattern expression in
              Exp.let_ ~loc Nonrecursive [letExpression] innerExpression
            in
            let innerExpression =
              List.fold_left makeLet
    ...
    | { pstr_loc
      ...
      :: restOfStructure -> (
        let jsxField, recordFieldsWithoutJsx =
          recordFields |> List.partition (fun ({txt}, _) -> txt = Lident "jsx")
        in
        ...
        | _ ->
            raise
              (Invalid_argument
                 "JSX: the file-level bs.config's {jsx: ...} config accepts \
...
let () =
  Driver.register ~name:"JSX" Migrate_parsetree.Versions.ocaml_406
