...
let jsxMapper () =
  ...
  (* changed jsoo: new function *)
  let rec recursivelyMakeNamedArgsForMakeProps list args = match list with
  | (label, loc, type_)::tl ->
    (recursivelyMakeNamedArgsForMakeProps tl (match (label, type_) with
    | (_, None) ->
      Pexp_fun (label, None, {
        ppat_desc = Ppat_var { txt =
          (match label with | Labelled str |  Optional str -> str | _ -> raise (Invalid_argument "This should never happen.")); loc };
        ppat_loc = loc;
        ppat_attributes = [];
      }, args)
    | (Optional l, Some ({ptyp_desc = Ptyp_constr ({txt = Lident "option"}, [type_])})) ->
      Pexp_fun (Optional l, None, {
        ppat_desc = Ppat_var { txt = l; loc };
        ppat_loc = loc;
        ppat_attributes = [];
      }, args)
    | (_, Some type_) ->
      Pexp_fun (Labelled label, None, {
        ppat_desc = Ppat_var { txt = label; loc };
        ppat_loc = loc;
        ppat_attributes = [];
      }, args)
    ))
  ...
  let makePropsValue fnName loc namedArgListWithKeyAndRef propsType =
    ...
    pval_prim = [""];
    (* changed jsoo: bs.obj by js *)
    pval_attributes = [({txt = "js"; loc = loc}, PStr [])];
    pval_loc = loc;
  } in

  (* changed jsoo: new function *)
  let makePropsBinding fnName loc namedArgListWithKeyAndRef propsType =
    let propsName = fnName ^ "Props" in
    let innerExpression = {
      pexp_desc =
       recursivelyMakeNamedArgsForMakeProps
         namedArgListWithKeyAndRef
         (Ast_404.Ast_helper.Typ.arrow
           Nolabel
           {
    ...
    Ast_404.Ast_helper.Vb.mk
      (Ast_404.Ast_helper.Pat.var ~loc {txt = propsName; loc})
        innerExpression
    (* {
      pval_name = {txt = propsName; loc};
      pval_type =
  ...
  let makePropsExternal fnName loc namedArgListWithKeyAndRef propsType =
    {
      pstr_loc = loc;
      (* changed jsoo: Pstr_value instead of Pstr_primitive *)
      pstr_desc = Pstr_value (Nonrecursive, [makePropsBinding fnName loc namedArgListWithKeyAndRef propsType])
    }
  in
  ...
  | ({
    ...
    let newStructure = {
      pstr with pstr_desc = Pstr_primitive {
        ...
        pval_name = { txt = "Bla"; loc = pstr_loc };
